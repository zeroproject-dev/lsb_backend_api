...
from src.models.video import TVIDEO
from src.utils.video import extract_points_of_video, generate_preview, get_duration
...
def create_video():
    ...
    try:
        os.makedirs(os.path.join(POINTS_FOLDER, name_without_extension))
    except:
        return jsonify({"message": "Error al verificar el video"}), 400

    for i, point in enumerate(points):
        points_path = os.path.join(POINTS_FOLDER, name_without_extension, str(i))
