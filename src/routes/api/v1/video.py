...
def get_video_word(word_id):
    videos_with_users: List[Tuple[TVIDEO, TUSER]] = (
        db.session.query(TVIDEO, TUSER)
        .join(TWORDVIDEO, TVIDEO.id == TWORDVIDEO.video_id)
        .join(TUSER, TVIDEO.uploaded_by == TUSER.id)
        .filter(TWORDVIDEO.word_id == word_id)
        .all()
    )
...
def create_video(word: str, word_id: int):
    ...
    new_video = TVIDEO()
    new_video.path = os.path.join(
        "static", "videos", video_name + "." + video_extension
    )
    new_video.preview = os.path.join("static", "images", video_name + ".jpg")
